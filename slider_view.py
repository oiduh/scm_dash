...
from dash import Dash, Input, Output, State, callback, callback_context, html
from dash.dcc import Checklist, Dropdown, Graph
...
dist_map = {"norm": norm, "log_norm": log_norm, "laplace": laplace, "uniform": uniform}
...
def checklist(distrs, all, fig):
    ...
    if len(selected) == 1:
        ...
        df_ = pd.DataFrame(np.array(choices).T, columns=selected)
        fig = ff.create_scatterplotmatrix(
            df=df_, diag="histogram", width=1200, height=1200
        )
